...
const CartList = () => {
    ...
    useEffect(() => {
        if (!isLoadingUser && !user) {
            showSwal("Error", "You need to be logged in to be able to view your cart.", "error");
            router.push("/");
        }
    ...
    const changeTotalItemCallback = useCallback(
        (productId: string, change: number) => {
            queryClient.setQueryData(["carts"], (oldData: ICart[] | undefined) => {
                ...
                if (updatedItem) {
                    fetchChangeTotalItemServerCallback(updatedItem.total, productId, user?._id);
                }
    ...
    return (
        <>
            ...
            <div className="w-full fixed bg-white bottom-0 flex border py-3 px-5 items-center justify-between">
                ...
                <Button onClick={addToCheckout}>Check Out</Button>
            </div>
        </>
    );
};
