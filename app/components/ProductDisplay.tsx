    ...
    return (
        <>
            ...
            <div
                ...
                <div
                    className="px-3 pb-3 mt-2 flex flex-col w-full"> {/* Set a fixed height and enable vertical scrolling */}
                    <p className="text-[14px] text-justify">{product?.description}</p>
                </div>
            </div>
            {/* Fixed Button at the Bottom */}
            <div
                className="fixed bottom-0 left-0 right-0 bg-white p-3 shadow-lg"> {/* Fixed positioning and styling for the button */}
                <Button onClick={addToCartCallback} className="w-full rounded-xl text-lg font-semibold py-6">{mutation.isPending ? "Adding To Cart..." : "Add To Cart"}</Button>
            </div>
        </>
    );
};
